...
beforeEach(async () => {
  ...
  const createParentRequest = await request(config.NETUNO_URL)
    .post("/post")
...
test("deleting a comment should decrement the parent post comments counter by 1", async () => {
  ...
  const beforeDeleteResponse = await request(config.NETUNO_URL)
  ...
  const afterDeleteResponse = await request(config.NETUNO_URL)
    .get(`/post?uid=${parentPostUid}`)
    .set("Authorization", `Bearer ${accessToken}`)
    .expect(200);
...
test("deleting a comment should not affect other comments on the same parent post", async () => {
  ...
  const createSecondCommentRequest = await request(config.NETUNO_URL)
    .post(`/post?parent=${parentPostUid}`)
    .set("Authorization", `Bearer ${accessToken}`)
    .set("Accept", "*/*")
    .set("Content-Type", "application/json")
