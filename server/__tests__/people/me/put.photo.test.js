...
test("should be able to upload an avatar", async () => {
  ...
  const beforeResponse = await fetch(`${config.NETUNO_URL}/people/me`, {
  ...
  const putResponse = await fetch(`${config.NETUNO_URL}/people/me`, {
    method: "PUT",
    headers: { "Authorization": `Bearer ${accessToken}` },
    body: formData
  });
  ...
  const afterResponse = await fetch(`${config.NETUNO_URL}/people/me`, {
    headers: { "Authorization": `Bearer ${accessToken}` }
  }).then(r => r.json());
